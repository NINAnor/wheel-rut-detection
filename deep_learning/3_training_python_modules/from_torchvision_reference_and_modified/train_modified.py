...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler,
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq,
        ...
        if (train_datatype_str == 'float16'):
            ...
            scaler.update()
        elif (train_datatype_str == 'float32'):
            output = model(image)
            loss = criterion(output, target)
            ...
            print('ERROR: unknown train_datatype_str', flush=True)
            return None

        metric_logger.update(loss=loss.detach().item(), lr=optimizer.param_groups[0]["lr"])
